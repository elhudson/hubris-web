...
import Feature, { Checkbox } from '@elements/feature'
import { CheckboxItem, SmallMod } from "@elements/text"
...
export default class Entry {
    ...
    matches(filter) {
        ...
        for (var key of props) {
            ...
            if (filter[key]==val || (Array.isArray(val) && val.includes(filter[key]))) {
                matches.push(true)
            } else {
                matches.push(false)
            }
...
export class Metadata extends Entry {
    ...
    applies(feature) {
        if (this.tree == feature.tree) {
            return true
        }
        else if (this.tree.split('/').includes(feature.tree)) {
            return true
        }
        else {
            return false
...
export class Effect extends Entry {
    ...
    dc_add(range, duration) {
        range == undefined && (range = { power: 1 })
        duration == undefined && (duration = { power: 1 })
        var raw = this.power * range.power * duration.power / 5
        if (raw < 1) { return 1 }
        else { return Math.ceil(raw) }
    }
