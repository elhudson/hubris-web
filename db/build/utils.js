...
export const groupBy = (list, get, subprop) => {
    const cats = _.cloneDeep(_.uniqBy(list.map((f) => get(f)), "id"));
    cats.forEach((cat) => {
...
export const not_relations = (table) => Object.keys(table[0]).filter((f) => !Array.isArray(table[0][f]));
...
export const complex_relations = (table) => relations(table).filter((f) => !simple_relations(table).includes(f));
export const relations = (table) => Object.keys(table[0]).filter((f) => Array.isArray(table[0][f]));
export const relationless = (table) => {
    return table.map((d) => Object.fromEntries(not_relations(table).map((n) => [n, d[n]])));
};
const always_hide = (entry, p) => !p.toLowerCase().includes("id") &&
    !_.isNull(entry[p]) &&
    !["title", "description", "id"].includes(p);
export const basic_props = (entry) => Object.keys(entry).filter((p) => always_hide(entry, p) && !Array.isArray(entry[p]) && !_.isObject(entry[p]));
export const single_link_props = (entry) => Object.keys(entry).filter((p) => always_hide(entry, p) && _.isObject(entry[p]) && !Array.isArray(entry[p]));
export const multi_link_props = (entry) => Object.keys(entry).filter((p) => always_hide(entry, p) && Array.isArray(entry[p]));
...
export const affordable = (feature, char, tbl = null) => {
    ...
    return _.isUndefined(feature === null || feature === void 0 ? void 0 : feature.xp) || feature.xp <= budget;
};
...
export const get_power_cost = ({ ranges, durations, effects }) => {
    const div = _.sumBy(ranges, "power") *
...
export function get_skill_xp({ backgrounds, int, skills }) {
    var _a;
    var xp = 0;
    const s = skills === null || skills === void 0 ? void 0 : skills.filter((s) => !(backgrounds === null || backgrounds === void 0 ? void 0 : backgrounds.map((s) => { var _a; return (_a = s === null || s === void 0 ? void 0 : s.skills) === null || _a === void 0 ? void 0 : _a.id; }).includes(s === null || s === void 0 ? void 0 : s.id)));
    const num_skills = (_a = s === null || s === void 0 ? void 0 : s.length) !== null && _a !== void 0 ? _a : 0;
